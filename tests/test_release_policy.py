...
class Version05PolicyTests(unittest.TestCase):
    ...
    def test_release_metadata_is_synchronized(self) -> None:
        self.assertEqual(holoforge.__version__, "0.6.0")
        citation = (ROOT / "CITATION.cff").read_text()
        changelog = (ROOT / "CHANGELOG.md").read_text()
        readme = (ROOT / "README.md").read_text()
        self.assertIn("version: 0.6.0", citation)
        self.assertIn("date-released: 2026-08-29", citation)
        self.assertIn("## [0.6.0] - 2026-08-29", changelog)
        self.assertIn("latest public release is `0.6.0`", readme)
